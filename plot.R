...
df <- read_feather('d.feather') %>%
    mutate(dttm = as.POSIXct(dttm, origin = orig, tz = "Europe/Warsaw"),
           ...
           umbrella = ifelse(precipInt > 0.5 & precipProb > 0.2, intToUtf8(utf8ToInt("\xe2\x98\x82")), NA),
           precipProb = ifelse(precipProb==0&lag(precipProb)==0&lead(precipProb)==0, NA, precipProb),
...
dd <- read_feather('d_daily.feather') %>%
    mutate(dttm = as.POSIXct(dttm, origin = orig, tz = "Europe/Warsaw"),
           ...
           day_x = as.POSIXct(ifelse(sunrise > Sys.time(), sunrise, Sys.time()), origin = orig, tz = "Europe/Warsaw")
           )
...
ggplot(df, aes(xmin = dttm-minutes(30), xmax = dttm+minutes(30))) +
    # day/night
    geom_rect(data = dd, aes(xmin = sunrise, xmax = sunset, ymin = -Inf, ymax = Inf), fill = 'yellow', alpha = 0.1) +
    ...
    # precip prob
    geom_line(aes(x = dttm, y = y1+precipProb*3), colour = "steelblue", size = 0.5, linetype = 2) + 
    # precip intensity
    ...
    # day label
    geom_label(data = dd, aes(x = day_x, y = y1+3, label = day), size = 2, colour = "black", alpha = 0.8, hjust = 0.3) +
    # sunrise
    ...
    # day length
    # geom_label(data = dd, aes(x = sunrise + (sunset - sunrise)/2, y = y0-1, label = paste(intToUtf8(utf8ToInt("\xe2\x86\x94")),d_len)), fill = 'black', size = 2, colour = "white", alpha = 0.8) +
    # theme
    coord_cartesian(ylim = c(y0-3, y1+3.5), xlim = c(min(df$dttm), max(df$dttm))) +
    scale_fill_distiller(palette = "Spectral", limits = c(n,m)) +
    scale_x_datetime(date_breaks = '3 hours', date_minor_breaks = '3 hours', date_labels = "%H:%M") +
